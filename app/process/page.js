...
export default function ProcessPage() {
  return (
    <div style={{ fontFamily: "Manrope, sans-serif", color: "#EFE7DA", background: "#221C15", minHeight: "100vh" }}>
      ...
      <div style={{ padding: "72px 64px 56px", display: "flex", flexDirection: "column", gap: "16px", maxWidth: "900px" }}>
        <span style={{ font: "600 11px Manrope, sans-serif", letterSpacing: ".32em", color: "oklch(0.74 0.08 78)" }}>HOW IT WORKS · 六步交付</span>
        <h1 style={{ margin: 0, font: "italic 500 76px/1.05 'Cormorant Garamond', serif" }}>Six steps, no surprises.</h1>
      ...
      <div style={{ padding: "0 64px 80px", display: "flex", flexDirection: "column" }}>
        {processDetailSteps.map((st) => (
          <div key={st.num} style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr", gap: "48px", padding: "46px 0", borderTop: "1px solid rgba(239,231,218,.12)", alignItems: "start" }}>
      ...
      <div style={{ background: "#EFE7DA", color: "#221C15", padding: "90px 64px", display: "flex", flexDirection: "column", gap: "44px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          <h2 style={{ margin: 0, font: "italic 500 50px/1.1 'Cormorant Garamond', serif" }}>Asked before every project.</h2>
        </div>
